...
class CalendarItemView extends View {
    ...
    public void selectDate(int position) {
        if (position >= 0 && position < mDates.size()) {
            if (mSelectedItemPosition != -1 && mCalendarViewDelegate.getSelectedDate()
                    == getDates().get(mSelectedItemPosition)) {
                return;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mSelectedItemPosition >= 0 && mSelectedItemPosition < mDates.size()) {
            ...
            if (!tabletSize)
                canvas.drawCircle(drawX, drawY, mItemWidth / 2.5f, mPaint);
            else
                canvas.drawCircle(drawX, drawY, mItemWidth / 4f, mPaint);
        }
        ...
        for (int i = 0; i < mDates.size(); i++) {
            ...
            } else if (date.getType() != Date.TYPE_THIS_MONTH) { //mRow==1为周模式，周模式不选择改变样式。
            ...
            if (mSelectedItemPosition != i &&
                    (mCalendarViewDelegate.isShowLunar() || mCalendarViewDelegate.isShowHoliday()
                            || mCalendarViewDelegate.isShowEventDot())) {
                mPaint.setColor(mCalendarViewDelegate.getBottomTextColor());
                ...
                if (text != null) {
                    //2018/2/24 bottomDrawY 的坐标运算优化。
                    float bottomDrawY = mFirstItemDrawY * 1.4f + row * (mItemHeight
                            + mItemHeightSpace);
                    //commented below code as we don't need lunar or holiday to set here
//                    canvas.drawText(text, itemDrawX, bottomDrawY, mPaint);
                }
            }

            /* added by PatelSanket on 29 Dec, 2020 */
            // set event dots code starts
            String text = null;
            if (mCalendarViewDelegate.isShowEventDot()) {
                if (mCalendarViewDelegate.getMapEventsDates().size() > 0) {
                    if (mCalendarViewDelegate.getMapEventsDates().containsKey(date.toString())) {
                        int value = mCalendarViewDelegate.getMapEventsDates().get(date.toString());
                        if (value > 1) {
                            text = "..";
                        } else if (value == 1) {
                            text = ".";
                ...
                if (text != null) {
                    float bottomDrawY = mFirstItemDrawY * 1.3f + row * (mItemHeight
                            + mItemHeightSpace);
                    canvas.drawText(text, itemDrawX, bottomDrawY, mPaint);
                }
            }
            // set event code dots ends

            if (date.isScheme()) {
                float schemeDrawY = row * (mItemHeight + mItemHeightSpace) + mItemHeight / 2;
                if (mSelectedItemPosition == i) {
                    mPaint.setColor(mCalendarViewDelegate.getSelectedSchemeColor());
                } else {
                    mPaint.setColor(mCalendarViewDelegate.getSchemeColor());
                }
                canvas.drawCircle(itemDrawX, schemeDrawY,
                        mCalendarViewDelegate.getSchemeRadius(), mPaint);
            }
        }
    }
